...
const ProfileCard = ({ ProfileCard }: { ProfileCard: ProfileCardEntity }) => {
  return (
    <div className="mx-4 md:mx-auto bg-card text-card-foreground rounded-3xl relative">
      <div
        className="w-full h-52 bg-cover bg-center rounded-t-3xl"
        style={{ backgroundImage: `url(${ProfileCard.bannerUrl})` }}
      ></div>

      <Avatar className="absolute top-44 left-5 border-4 border-white w-[136px] h-[136px]">
        <AvatarImage src={ProfileCard.avatarUrl} className="object-cover"/>
        ...
        <div>
          <div className="flex">
            <h1 className="flex items-center text-2xl gap-2">
              <span>{ProfileCard.fullname}</span>
              <Icon
          ...
          <div className="flex flex-wrap gap-5 mt-3">
            <div>
              ...
              <p className="text-sm font-medium">{ProfileCard.toursConductedNumber} тура</p>
            </div>

            <div>
              <p className="text-xs font-light text-gray-400">Опубликовано</p>
              <p className="text-sm font-medium">
                {ProfileCard.toursPublishedNumber} туров
              </p>
            </div>

            <div>
              <p className="text-xs font-light text-gray-400">Рейтинг</p>
              <p className="text-sm font-medium">{ProfileCard.score}</p>
            </div>
            ...
            <div>
              ...
              <p className="text-sm font-medium">{ProfileCard.touristsGuidedNumber}</p>
            </div>
          </div>
        </div>
