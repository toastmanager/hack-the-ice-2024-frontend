        ...
        return Array.from({ length: routeform.routesCount }, (_, index) => (
            <div key={index} className="space-y-4 border-b-2 border-[#1A1A1A33] mt-2">
                ...
                <div className="flex items-center gap-x-4">
                    <Input type="email" placeholder="Email" className="mt-1 w-2/3 h-10" />
                    <div className="py-2 px-2 bg-[#F2F2F2] items-center rounded-md flex w-1/3 h-10">
                        <span className="text-xs w-full">{routeform.countdown}</span>
                        <div className="flex items-center space-x-4">
                            <button onClick={() => decrementSeat(index)} className="bg-white py-1 px-2 w-6 rounded-md">-</button>
                            <h1>{seatCounts[index]}</h1>
                            <button onClick={() => incrementSeat(index)} className="bg-white py-1 px-2 w-6 rounded-md">+</button>
                        </div>
                    </div>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <label
                        ...
                        className="flex flex-col items-center justify-center h-64 border-2 border-gray-300 border-dashed rounded-lg cursor-pointer bg-[#F2F2F2]"
                    >
    ...
    return (
        <div className="w-2/3">
            ...
            <div className='bg-card text-card-foreground w-full p-6 space-y-4 rounded-b-2xl'>
                <Button variant="outline" className="w-full text-primary bg-primary text-white">
